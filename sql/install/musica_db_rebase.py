...
A timestamped backup is created before any database changes.
"""
...
def database_is_current(connection):
    """Return True if the database already contains the V1.1 schema."""
    ...
    trigger_names = {
        ...
        for row in connection.execute(
            ...
            WHERE type = 'trigger'
              AND name IN (
                  'trg_recordings_year_ins',
                  'trg_recordings_year_upd'
              )
            """
        ).fetchall()
    }
...
def create_display_view(connection):
    connection.execute(
        """
...
def main():
    ...
    try:
        ...
        # The V1.1 table is built alongside the original table.
        ...
        # v_recordings_display depends on recordings and must be removed
        # before the old recordings table can be dropped.
        connection.execute(
        ...
        create_display_view(connection)
